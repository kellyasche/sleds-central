...
-   *LimitedEnglishProficiency:* There are only 1.5% (2,217) individuals in this category.
-   *HomelessIndicator:* There are only .7% (1,013) individuals that indicated that they were homeless at one point between 10th and 12th grade.
-   *sat.taken:* Only 1% (1,440) have taken the SAT
-   *ps.grad.Institute:*
...
-   grad.InstitutionSector: combine categories 3, 5, 6, 7, 8 and 9 into one category. This means that the following institution sectors will be relabeled as "11";
...
master.ind.qual.var <- ind.qual.var %>%
  select(-PersonID, -LimitedEnglishProficiencyIndicator, -HomelessIndicator, -sat.taken) %>%
...
-   Sum of cos2 for all dimensions
    ...
    -   Moderate representation (between .2 and .5)
        -   RaceEthnicity - Black, Hispanic, Asian/PI
    -   Below .2
        -   None

**Dimension 1:** Language barriers and identifies as black

**Dimension 2:** Pre- or post-COVID grad

**Dimension 3:** Special education needed or not

**Dimension 4:** Gender and American Indian
...
1.  Language barriers and ethnicity (black and Hispanic) and relationship to poverty and special education needs
2.  Pre- or post-COVID grad
3.  Special education, American Indian and Asian/PI, poverty, non-English speaker at home and gender
4.  American Indian, gender differences and poverty
...
The values below indicate that the first two dimensions explain 77% of the variance. Examining the cos2 values for each dimension might allow us to eliminate one of these variables.
...
The first two dimensions explain 64% of the variation but it takes up to 7 dimensions to surpass 75%.
...
Let's take a look to see which variables are going to be important. From the total cos^2^ we can see that ther are a few that just aren't make the cut (below .2);
...
-   Keep all - modify following
    -   ps.grad.InstitutionSector: categories 2, 10, 11 & NA become "11"
    -   highest.cred.level: categories Master's degree or higher and Bachelor's degree become "Bachelor's degree or higher".

<br>

```{r final master}
master.final.after.mca <- master.ind.qual.var %>%
  mutate(ps.grad.InstitutionSector = ifelse(ps.grad.InstitutionSector %in% c("2", "10", "11") | is.na(ps.grad.InstitutionSector), "11", ps.grad.InstitutionSector),
