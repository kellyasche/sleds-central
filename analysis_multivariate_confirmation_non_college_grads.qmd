...
# Does CTE cut through?

I want to see if CTE can eliminate some of these barriers highlighted in the previous models. We will first run the

```{r}
cte.master.model <- master %>%
  filter(ps.grad == "No") %>%
  select(PersonID, pseo.participant, cte.achievement, RaceEthnicity, economic.status, SpecialEdStatus, ap.exam, Dem_Desc, avg.wages.pct.state, MCA.M, MCA.R, MCA.S, ACTCompositeScore, grad.year.5) %>%
  filter(grad.year.5 != "After 2023",
         grad.year.5 != "Attending ps") %>%
  mutate(grad.year.5 = ifelse(grad.year.5 %in% c("Meaningful emp region", "Meaningful emp County", "Meaningful emp EDR"), "Meaningful emp region", grad.year.5),
         grad.year.5 = str_replace(grad.year.5, ", not attending ps", ""),
...
model_cte_econ <- glm(
  grad.year.5 ~
    cte.achievement * economic.status +
    SpecialEdStatus +
    RaceEthnicity +
    Dem_Desc +
    ACTCompositeScore +
    ...
    MCA.M + MCA.R + MCA.S,
  family = binomial,
  data = cte.master.model
)

model_cte_econ

or_plot <- tidy(model_cte_econ, conf.int = TRUE, exponentiate = TRUE) %>%
  filter(term != "(Intercept)") %>%
  mutate(
    # Make labels nicer (edit these replacements to match your exact factor labels)
    term = str_replace_all(term, c(
...
or_plot <- tidy(model_cte_sped, conf.int = TRUE, exponentiate = TRUE) %>%
  filter(term != "(Intercept)") %>%
  mutate(
    # Make labels nicer (edit these replacements to match your exact factor labels)
    term = str_replace_all(term, c(
...
or_plot <- tidy(model_cte_race, conf.int = TRUE, exponentiate = TRUE) %>%
  filter(term != "(Intercept)") %>%
  mutate(
    # Make labels nicer (edit these replacements to match your exact factor labels)
    term = str_replace_all(term, c(
