...
Now I'm going to prep the CTE demographics dataset to join with the master. I will first load the entire CTE demographics dataset, clean it up, and then join with my master/Central graduate dataset.
...
When joined with the master file, there were `r comma(cte.concentrator.cat %>% filter(cte.achievement != "No CTE") %>% nrow(), accuracy = 1)` that were either a CTE participant or concentrator. This is below the \~115,556,000 that took at least one CTE course according to the CTE enrollment dataset. This could mean that there roughly 20,000 students that took a CTE course but did not graduate.
...
The cross-tabs show a significant difference between the percentage of students by categorization and EDRs. The largest difference is EDR 7E where 50% of the graduates were a CTE concentrator or completor.
...
The cross tabulations show that there is a statistically significant difference in the percent of graduates in each CTE category by county. The highest percentage of No CTE graduates is Sherburne County with 42.5% and Pine not far behind with 40%. The counties with the highest percentage of CTE completors/concentrators are;
