...
grad.emp.location.match <- grad.locations %>%
  ...
  mutate(region.match = new.mn.emp.record,
         new.region.match = ifelse(region.match == "Meaningful employment" & emp.pr %in% c("Central"), "Meaningful emp - match", region.match),
...
At 5 years after graduation, the percentage of graduates that have meaningful employment in their EDR is around 20%. The percentage with meaningful employment outside of their EDR is 28% while 24% have no MN employment record.
...
Half the graduates have meaningful employment in Minnesota while about 24% have no MN employment record.
