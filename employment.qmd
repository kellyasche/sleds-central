...
emp.original.locations <- employment.updated %>%
  ...
  select(PersonID, CalendarYear, EmploymentOrganizationID, CountyName, edr, project.pr) %>% 
  distinct(PersonID, CalendarYear, EmploymentOrganizationID, CountyName, edr, project.pr) %>%
  rename(employment.county = CountyName,
         emp.edr = edr,
         emp.region = project.pr) %>%
...
grad.emp.location.match <- grad.locations %>%
  ...
  mutate(county.match = ifelse(employment.county == hs.county.name, 1,
         ...
         region.match = ifelse(emp.edr %in% c("EDR 5 - North Central", "EDR 7E- East Central", "EDR 7W- Central"), 1,
                               ifelse(emp.edr == "Unknown", 0, -1)),
         state.match = 1) %>%
  group_by() %>%
